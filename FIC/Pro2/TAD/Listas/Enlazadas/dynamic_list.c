#include "dinamicaListaEnlazada.h"
...
bool existItem (tItemL d, tList L){
  return(findItem(d,L) != LNULL)
}

bool equalList(tList L tList M){
  tPosL p, q;
...
void removeOcurrence(tItemL d, tList *L){
  tPosL p,q;
  if(getItem(first(*L); *L) == d){
    p = *L;
    *L = next(p, *L);
  }
  else if(!isEmptyList(*L)){
    for(q = first(*L); getItem(q, *L) != d) && q != LNULL; q = next(q *L));
    p = q;
    q = previus(p, *L);
  }
  deleteposition(p,*L);
